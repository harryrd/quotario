...
export const useInvoiceForm = (userId: string | undefined) => {
  ...
  useEffect(() => {
    if (userSettings && userSettings.documentTemplates?.invoice) {
      try {
        const templateFields = userSettings.documentTemplates.invoice.fields;
  ...
  const handleSubmitInvoice = async () => {
    ...
    const startNumber = userSettings?.invoiceStartNumber || '1001';
